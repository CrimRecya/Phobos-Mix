...
Spawner.RecycleFLH=                          ; FLH坐标，请写完整，子机的回收位置
Spawner.RecycleOnTurret=false                ; 布尔值，回收位置 `Spawner.RecycleFLH` 的计算是否以炮塔坐标轴为基准
```
...
```{note}
- 关于2个功能快捷键：
  - 功能快捷键默认名为 `Assign the rally point` 与 `Assign 2nd rally point`，可以在csf中添加名为 `TXT_ASSIGN_RALLY_POINT` 与 `TXT_ASSIGN_2ND_RALLY_POINT` 和 `TXT_ASSIGN_RALLY_POINT` 与 `TXT_ASSIGN_2ND_RALLY_POINT_DESC` 的项目来分别重写其在快捷键页面中的选项名称和详细说明
...
- 仅优化，无ini
- 原本游戏中，单位隐身时会声明指针无效，这导致的问题包括但不限于子机导弹丢失所属无法为本体提供经验
...
```ini
[General]                                    ; 全局通用设置
InTransportInfantryAmmoFix=false             ; 布尔值，是否启用修复
```
...
```ini
[General]                                    ; 全局通用设置
UpdateInLimbo.Occupier=false                 ; 布尔值，驻军是否保持更新
UpdateInLimbo.NormalPassenger=false          ; 布尔值，非对外开火的载员是否保持更新
UpdateInLimbo.Parasite=false                 ; 布尔值，寄生虫是否保持更新
```
...
Refinery.UseNormalActiveAnim=false           ; 布尔值，是否使用常规工作动画逻辑
```
...
Trajectory=                                  ; Trajectory类型，以下为【Engrave】（【不需要】武器上的参数，只会在刚开火时触发步兵攻击帧，且如果可被拦截会很诡异，不影响炮塔旋转，建议速度为40）
...
Trajectory.Engrave.ProximityDirect=no        ; 布尔值，撞击伤害是否不引爆弹头而直接对目标造成伤害
Trajectory.Engrave.ProximityMedial=no        ; 布尔值，撞击伤害是否不在目标位置引爆，而是直接在抛体位置引爆
Trajectory.Engrave.ProximityAllies=no        ; 布尔值，撞击伤害是否对盟友生效
Trajectory.Engrave.ProximityFlight=no        ; 布尔值，撞击伤害对空中单位是否生效
Trajectory.Engrave.ProximitySuicide=no       ; 布尔值，撞击次数耗尽抛体是否需要消失
Trajectory.Engrave.ConfineOnGround=yes       ; 布尔值，抛体是否始终吸附地面，否则会保持在初始高度
```
...
Trajectory.Disperse.UniqueCurve=no           ; 布尔值，是否启用模拟钱学森弹道，启用后以下22项及 `Trajectory.Speed` 无效，最高速度固定为192（应该不会有人范围很小的情况下还拿这个做弹道吧）
...
Trajectory.Disperse.SuicideAboveRange=-3.0   ; 浮点型，抛体本体在飞行距离达到该值后直接自爆，为0代表不因此自爆，当设置为负数时其绝对值代表初始距离的倍数
Trajectory.Disperse.SuicideShortOfROT=no     ; 布尔值，抛体本体是否在发现转向能力不足时自爆
...
Trajectory.Tracing.PeacefulVanish=no         ; 布尔值，抛体在最后将要引爆时是否直接消失，即不产生动画和造成伤害
Trajectory.Tracing.TraceTheTarget=yes        ; 布尔值，抛体追踪的目标是否为抛体的目标，否则为追踪发射者，当发射者死亡时，抛体也会一同引爆
...
Trajectory.Tracing.WeaponFromFirer=yes       ; 布尔值，抛体发射的追踪武器是否由单位朝抛体发射，否则是由抛体朝目标发射
...
```ini
[AudioVisual]                                ; 视听通用设置
VisualScatter.Min=0.03                       ; 浮点型，最小散布距离，不随距离增大而增大
VisualScatter.Max=0.13                       ; 浮点型，最大散布距离，不随距离增大而增大

[SOMEWEAPON]                                 ; 任意武器
VisualScatter=no                             ; 布尔值，是否启用额外散射，不影响抛射体的实际位置
```

```{note}
- 同时修复了激光、电弧、辐射波绘制时不支持抛射体散射（包括 `FlakScatter=yes` 和 `Inaccurate=yes`）的问题
```
...
; ------------------------------------------ ; 弹道抛体的全通用基本功能
Inaccurate=no                                ; 布尔值，抛体是否启用随机散布，与原版略有不同，不同弹道类型存在不同含义
;                                            ; Straight - 抛体在目标位置的随机散布
;                                            ; Bombard - 抛体在目标位置的随机散布
;                                            ; Missile - 抛体在发射方向的随机偏离
;                                            ; Engrave - 抛体在目标位置的随机散布
;                                            ; Parabola - 抛体在目标位置的随机散布
;                                            ; Tracing - 抛体在发射位置的随机散布
...
;                                            ; Straight - 抛体能够存在的时间，时间结束时抛体会引爆
;                                            ; Bombard - 抛体能够存在的时间，时间结束时抛体会引爆
;                                            ; Missile - 抛体能够存在的时间，时间结束时抛体会引爆
...
;                                            ; Parabola - 抛体能够存在的时间，时间结束时抛体会引爆
;                                            ; Tracing - 抛体锁定的持续时间，时间结束时抛体会引爆，如果为非正数，则使用武器 `ROF` - 10 作为持续时间，持续时间最低为1
Trajectory.TolerantTime=-1                   ; 整数型，抛体失去目标闲置时的容忍时间，抛体在失去目标超过这么长时间后会引爆，若为0，则会在切换目标时直接引爆
Trajectory.BulletROT=-1                      ; 整数型，抛体图像的转动速度，不影响移动方向，为负数时跟随移动方向，为0时会始终朝向目标，为正数时会根据该速度向目标转动，如果 `Trajectory.Tracing.BulletSpin` 为是则由正负决定自转方向，数值决定速度
Trajectory.BulletSpin=no                     ; 布尔值，抛体图像是否会一直旋转
Trajectory.BulletStable=no                   ; 布尔值，抛体图像是否在发射后不再转动
Trajectory.BulletOnPlane=yes                 ; 布尔值，抛体图像的旋转是否始终在水平面上
Trajectory.RetargetRadius=0                  ; 浮点型，抛体本体在失去目标后重新搜索新目标的半径，若未到达过 `Trajectory.OffsetCoord` ，则以原目标最后的坐标为圆心，否则以自身前方这个距离的坐标为圆心，
;                                            ; 为0代表不启用重新搜索，会攻击原本的位置，为负数则会直接原地自爆，当原目标在空中时，新目标也只会在空中寻找，当原目标在地上时，新目标也只会在地上寻找，不同弹道类型存在不同动作
;                                            ; Straight - 仅更换目标
;                                            ; Bombard - 仅更换目标
;                                            ; Missile - 更换目标，转向目标
;                                            ; Engrave - 仅更换目标
;                                            ; Parabola - 仅更换目标
;                                            ; Tracing - 更换目标，转向目标
Trajectory.Synchronize=yes                   ; 布尔值，抛体的目标是否同步发射者的目标，如果不同步，抛体不会直接更新目标，优先级高于 `Trajectory.RetargetRadius` ，不同弹道类型存在不同动作
;                                            ; Straight - 仅更换目标
;                                            ; Bombard - 仅更换目标
;                                            ; Missile - 更换目标，转向目标
;                                            ; Engrave - 仅更换目标
;                                            ; Parabola - 仅更换目标
;                                            ; Tracing - 更换目标，转向目标
Trajectory.PeacefulVanish=                   ; 布尔值，抛体在最后引爆时是否直接消失，没有动画和伤害，默认值为 `Trajectory.ProximityImpact` 不等于0 或 `Trajectory.DisperseWeapons` 不为空
...
Trajectory.UseDisperseBurst=no               ; 布尔值，计算 `Trajectory.RotateCoord` 时是否按照扩散它的上级扩散抛体的 `Trajectory.DisperseBurst` 的值而非武器的 `Burst`
...
Trajectory.EdgeAttenuation=1.0               ; 浮点型，射程边缘修正比，抛体飞行到最大射程造成的伤害与距离一格内造成伤害的【比例】，之间是线性的，不支持负数（包括自身伤害、`Trajectory.PassDetonateDamage` 和 `Trajectory.ProximityDamage`）
Trajectory.CountAttenuation=1.0              ; 浮点型，伤害次数衰减率，即每次造成伤害后下一次造成伤害会是本次伤害的多少倍，不支持负数（包括自身伤害、`Trajectory.PassDetonateDamage` 和 `Trajectory.ProximityDamage`）
...
Trajectory.DisperseDelays=                   ; 整数型列表，抛体在发射对应武器后需要等待多久才能进行下一次发射，至少为1，如果数量少于 `Trajectory.DisperseWeapons` ，将使用该列表的最后一项
Trajectory.DisperseCounts=                   ; 整数型列表，抛体需要扩散几次对应武器，为零会直接计算冷却而不发射，负数视为无限，如果数量少于 `Trajectory.DisperseWeapons` ，将使用该列表的最后一项
Trajectory.DisperseCycle=-1                  ; 整数型，抛体能够发射几轮武器，为零不会发射武器，负数视为无限
...
;                                            ; Parabola - 否
;                                            ; Tracing - 是
Trajectory.DisperseFaceCheck=no              ; 布尔值，抛体在发射武器前是否检查图像朝向，如果 `Trajectory.BulletSpin=yes` 或 `Trajectory.BulletROT` 为负数时无视该项强制不检查
...
Trajectory.DetonationDistance=0.4            ; 浮点型，抛体距离目标多远距离就会引爆，不同弹道类型存在不同含义
;                                            ; Straight - 当抛体 `Trajectory.Straight.PassThrough=yes` 时，则若此值为正则抛体在距离【发射者】这个值的距离的位置引爆，为负则抛体在穿过【目标点】这个值的绝对值的位置引爆，为零视为无穷远
;                                            ; Bombard - 抛体在距离目标多远距离就会引爆
;                                            ; Missile - 抛体在距离目标多远距离就会引爆
;                                            ; Parabola - 抛体在水平方向上的速度，且仅用于模式0、模式3和模式5
Trajectory.LeadTimeCalculate=no              ; 布尔值，抛体在瞄准时是否计算目标的提前量，不同弹道类型存在不同含义
;                                            ; Straight - 抛体的目标位置是否计算目标的提前量，计算目标的提前量会在开火时延迟1帧发射，同时若抛体的速度过低，则会无法生效
;                                            ; Bombard - 抛体的目标位置是否计算目标的提前量，若 `Trajectory.Bombard.NoLaunch=yes` 且 `Trajectory.Bombard.FreeFallOnTarget=yes` 则会同时影响到下降阶段的初始位置
;                                            ; 若 `Trajectory.Bombard.NoLaunch=no` 则计算目标的提前量会在开火时延迟1帧发射，同时若抛体的速度过低，则会无法生效
;                                            ; Missile - 抛体在运动过程中是否持续计算目标的提前量
;                                            ; Parabola - 抛体的目标位置是否计算目标的提前量，计算目标的提前量会在开火时延迟1帧发射且增加性能消耗，同时若抛体的速度过低，则会无法生效
Trajectory.OffsetCoord=0,0,0                 ; FLH坐标，请写完整，抛体目标位置的额外偏移，不同弹道类型存在不同含义
;                                            ; Straight - 抛体会瞄准目标的这个相对坐标进行攻击，支持在此基础上启用随机散布
;                                            ; Bombard - 抛体会瞄准目标的这个相对坐标进行攻击，支持在此基础上启用随机散布
;                                            ; Missile - 抛体初始会朝着这个坐标飞行，而后转向，为0,0,0时会直接朝着目标前进
;                                            ; Parabola - 抛体会瞄准目标的这个相对坐标进行攻击，支持在此基础上启用随机散布
;                                            ; Tracing - 抛体会追踪其追踪目标的这个位置，由 `Trajectory.Tracing.TraceMode` 决定具体位置
...
Trajectory.Missile.TurningAngle=10.0         ; 浮点型，抛体转向速度，即抛体的方向每帧最多能旋转多少角度
...
Trajectory.Missile.CruiseUnableRange=5       ; 浮点型，距离目标多远时结束巡航阶段，不再保持巡航高度，并开始朝向目标前进，如果在达到 `Trajectory.OffsetCoord` 时已经小于该值，会跳过巡航阶段（如果此值过小，则可能导致抛体永久处在巡航状态）
