...
- 仅修复，无ini
- 在原版游戏中，维修武器索敌距离强制为512，而且无法获取空中的目标，现在它们都可以正常工作了（索敌距离的改动和wic同款功能可以共存）
...
```ini
[General]                                    ; 全局通用设置
ExtraTargeting.OnLoseTarget=false            ; 布尔值，当单位处于攻击任务且失去目标时是否额外索敌一次
ExtraTargeting.OnStopCommand=false           ; 布尔值，当命令单位停止时是否额外索敌一次
```
