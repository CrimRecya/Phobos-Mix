...
```ini
[SOMETECHNO]                                 ; 任意步兵、车辆、飞机或建筑
DefaultVisualCharacterToSelf=0               ; 整数型，0-5，对自己的默认透明度
DefaultVisualCharacterToAlly=0               ; 整数型，0-5，对友方的默认透明度
DefaultVisualCharacterToEnemy=0              ; 整数型，0-5，对敌人的默认透明度
```
...
```ini
[SOMETECHNO]                                 ; 任意步兵、车辆、飞机或建筑
IgnoreByMouse.ToSelf=no                      ; 布尔值，己方鼠标是否忽略
IgnoreByMouse.ToAlly=no                      ; 布尔值，友方鼠标是否忽略
IgnoreByMouse.ToEnemy=no                     ; 布尔值，敌方鼠标是否忽略
...
; ------------------------------------------ ; Straight 类型独有功能
Trajectory.RotateCoord=0                     ; 浮点型，是否按照武器的 `Burst` 在以 `Trajectory.Straight.OffsetCoord` 为角平分线的角度内旋转抛体发射方向，为0代表不启用该逻辑（负值会反转旋转方向）
Trajectory.OffsetCoord=0,0,0                 ; FLH坐标，请写完整，抛体目标位置的额外偏移，抛体会瞄准目标的这个相对坐标进行攻击，支持在此基础上启用随机散布
Trajectory.AxisOfRotation=0,0,1              ; FLH坐标，请写完整，计算 `Trajectory.Straight.RotateCoord` 时的旋转轴，旋转轴会随单位朝向或目标位置到发射位置的向量旋转，长度不重要，只看方向（相反的向量也会反转旋转方向）
Trajectory.LeadTimeCalculate=no              ; 布尔值，抛体在瞄准时是否计算目标的提前量，计算目标的提前量会在开火时延迟1帧发射，同时若抛体的速度过低，则会无法生效
Trajectory.DetonationDistance=0.4            ; 浮点型，抛体距离目标多远距离就会引爆，当抛体 `Trajectory.Straight.PassThrough=yes` 时，则若此值为正则抛体在距离【发射者】这个值的距离的位置引爆，为负则抛体在穿过【目标点】这个值的绝对值的位置引爆，为零视为无穷远
...
; ------------------------------------------ ; Bombard 类型独有功能
Trajectory.RotateCoord=0                     ; 浮点型，是否按照武器的 `Burst` 在以 `Trajectory.Bombard.OffsetCoord` 为角平分线的角度内旋转抛体发射方向，为0代表不启用该逻辑（负值会反转旋转方向）
Trajectory.OffsetCoord=0,0,0                 ; FLH坐标，请写完整，抛体目标位置的额外偏移，抛体会瞄准目标的这个相对坐标进行攻击，支持在此基础上启用随机散布
Trajectory.AxisOfRotation=0,0,1              ; FLH坐标，请写完整，计算 `Trajectory.Bombard.RotateCoord` 时的旋转轴，旋转轴会随单位朝向或目标位置到发射位置的向量旋转，长度不重要，只看方向（相反的向量也会反转旋转方向）
Trajectory.LeadTimeCalculate=no              ; 布尔值，抛体在瞄准时是否计算目标的提前量，若 `Trajectory.Bombard.NoLaunch=yes` 且 `Trajectory.Bombard.FreeFallOnTarget=yes` 则会同时影响到下降阶段的初始位置，若 `Trajectory.Bombard.NoLaunch=no` 则计算目标的提前量会在开火时延迟1帧发射，同时若抛体的速度过低，则会无法生效
Trajectory.DetonationDistance=0.4            ; 浮点型，抛体距离目标多远距离就会引爆，若为正数，代表抛体在距离目标多远距离内就会引爆，若为负数，代表若抛体目标可以移动则会将目标直接丢失转为攻击其所在位置的地面格子，若为零则关闭功能
Trajectory.TargetSnapDistance=0.5            ; 浮点型，抛体引爆时，当前目标在多近的距离内弹头会吸附过去
Trajectory.EarlyDetonation=no                ; 布尔值，抛体在 `Trajectory.Bombard.DetonationHeight` 启用时，是否在上升段高于该高度时引爆，否则在下降段低于该高度时引爆
Trajectory.DetonationHeight=-1               ; 整数型，抛体位于什么高度时提前引爆，与 `Trajectory.Bombard.EarlyDetonation` 组合使用，为负数时不生效
...
; ------------------------------------------ ; Missile 类型独有功能
Trajectory.RotateCoord=0                     ; 浮点型，是否按照武器的 `Burst` 在以 `Trajectory.Missile.PreAimCoord` 为角平分线的角度内旋转抛体发射方向，为0代表不启用该逻辑（负值会反转旋转方向）
Trajectory.OffsetCoord=0,0,0                 ; FLH坐标，请写完整，抛体目标位置的额外偏移，抛体会瞄准目标的这个相对坐标进行攻击，支持在此基础上启用随机散布，注意在发射时就计算完毕，之后不会随双方坐标变化旋转
Trajectory.AxisOfRotation=0,0,1              ; FLH坐标，请写完整，计算 `Trajectory.Missile.RotateCoord` 时的旋转轴，旋转轴会随单位朝向或目标位置到发射位置的向量旋转，长度不重要，只看方向（相反的向量也会反转旋转方向）
Trajectory.LeadTimeCalculate=yes             ; 布尔值，抛体在瞄准时是否计算目标的提前量，抛体在运动过程中是否持续计算目标的提前量
Trajectory.DetonationDistance=0.4            ; 浮点型，抛体距离目标多远距离就会引爆，抛体在距离目标多远距离就会引爆
Trajectory.TargetSnapDistance=0.5            ; 浮点型，抛体引爆时，当前目标在多近的距离内弹头会吸附过去
...
; ------------------------------------------ ; Parabola 类型独有功能
Trajectory.RotateCoord=0                     ; 浮点型，是否按照武器的 `Burst` 在以 `Trajectory.Parabola.OffsetCoord` 为角平分线的角度内旋转抛体发射方向，为0代表不启用该逻辑（负值会反转旋转方向）
Trajectory.OffsetCoord=0,0,0                 ; FLH坐标，请写完整，抛体目标位置的额外偏移，抛体会瞄准目标的这个相对坐标进行攻击，支持在此基础上启用随机散布
Trajectory.AxisOfRotation=0,0,1              ; FLH坐标，请写完整，计算 `Trajectory.Parabola.RotateCoord` 时的旋转轴，旋转轴会随单位朝向或目标位置到发射位置的向量旋转，长度不重要，只看方向（相反的向量也会反转旋转方向）
Trajectory.LeadTimeCalculate=no              ; 布尔值，抛体在瞄准时是否计算目标的提前量，抛体的目标位置是否计算目标的提前量，计算目标的提前量会在开火时延迟1帧发射且增加性能消耗，同时若抛体的速度过低，则会无法生效
Trajectory.DetonationDistance=0.4            ; 浮点型，抛体距离目标多远距离就会引爆，若为正数，代表抛体在距离目标多远距离内就会引爆，若为负数，代表若抛体目标可以移动则会将目标直接丢失转为攻击其所在位置的地面格子，若为零则关闭功能
Trajectory.TargetSnapDistance=0.5            ; 浮点型，抛体引爆时，当前目标在多近的距离内弹头会吸附过去
Trajectory.EarlyDetonation=no                ; 布尔值，抛体在 `Trajectory.Parabola.DetonationHeight` 启用时，是否在上升段高于该高度时引爆，否则在下降段低于该高度时引爆
Trajectory.DetonationHeight=-1               ; 整数型，抛体位于什么高度时提前引爆，与 `Trajectory.Parabola.EarlyDetonation` 组合使用，为负数时不生效
